...
func (p Problem) Answer() {
	fmt.Println("Answer to problem " + strconv.Itoa(p.ID) + ": " + p.Solve())
}

// Test ensures that the answer is correct. Will fail until problem is solved.
func (p Problem) Test(t *testing.T) {
	assert.Equal(t, p.CorrectAnswer, p.Solve(), "These should be equal. Either it hasn't been solved or something broke.")
}
